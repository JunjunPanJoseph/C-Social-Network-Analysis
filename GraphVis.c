...
char* nodeTmp = (char *)"    {id:'%d',label:'%d',x:%f,y:%f, size:0.2, color: getColor(%f,gradient)}";
char* edgeTmp = (char *)"    {source:'%d',target:'%d',id:'%d',label:'%d',type: 'curvedArrow'}";

NodeValues makeBlankNodes(int nV) {
		NodeValues result;
		result.noNodes = nV;
		result.values = (double *)calloc(sizeof(double),nV);
		int i = 0;
